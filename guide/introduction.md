...
- **写入速度** 比 HashMap 快 **1.95 倍**（只写索引，不写数据）
- **百万级读取吞吐** 达到 176 万 ops/s，满足绝大多数业务场景
- 零拷贝序列化，原始类型直接内存布局
- 基于 Unsafe API 的底层优化
...
- ✅ 写入速度快 **1.95 倍**（只写索引，不写数据）
- ⚠️ 读取速度约为 HashMap 的 1/10（因涉及序列化，但 176 万 ops/s 仍能满足绝大多数场景）
