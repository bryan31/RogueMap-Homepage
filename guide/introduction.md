...
它基于内存映射文件（mmap）提供四种堆外数据结构，目标是让你在保持 Java API 易用性的同时，获得更低 GC 压力与可持久化能力。
...
- TTL 数据过期，无需外部缓存即可自动淘汰过期数据。
- 自动检查点（AutoCheckpoint），按时间或操作次数自动持久化。
- 超低堆索引（LowHeapStringIndex），将索引也移至堆外，进一步降低 GC 压力。
